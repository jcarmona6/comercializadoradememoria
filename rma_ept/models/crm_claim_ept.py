...
class CRMClaim(models.Model):
    ...
    def _get_default_company(self):
        ...
        if not company_id:
            raise UserError(_('There is no default company !'))
        return company_id
    ...
    def _valid_field_parameter(self, field, name):
        return name in ['size', 'index'] or super()._valid_field_parameter(field, name)
    ...
    def unlink(self):
        ...
        for record in self:
            if record.state != 'draft':
                raise UserError(_("Claim cannot be delete once it Processed."))
    ...
    def set_to_draft(self, id=False):
        ...
        if ticket_claim_id.return_picking_id and ticket_claim_id.return_picking_id.state != 'draft':
            if ticket_claim_id.return_picking_id.state in ['cancel', 'done']:
                raise UserError("Claim cannot be move draft state once it Receipt is done or cancel.")
            else:
    ...
    def process_claim(self):
        ...
        if self.state != 'process':
            raise UserError("Claim can't process.")
        if self.return_picking_id.state != 'done' and not self.is_rma_without_incoming:
            raise UserError("Please first validate Return Picking Order.")
        if self.internal_picking_id and self.internal_picking_id.state != 'done':
            raise UserError("Please first validate Internal Transfer Picking Order.")
        return_lines = []
        refund_lines = []
        do_lines = []
        so_lines = []
    ...
    def action_rma_send(self):
        ...
        try:
            template_id = self.env.ref('rma_ept.mail_rma_details_notification_ept')
        except ValueError:
            template_id = False
        try:
